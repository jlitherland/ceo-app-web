/**
 * PDF Parser Utility - IMPROVED VERSION
...
/**
 * Clean extracted text from PDF - ENHANCED VERSION
 */
...
/**
 * Validate PDF file - ENHANCED VERSION
 */
...
export async function isPDFSearchable(file: File): Promise<boolean> {
  try {
    ...
    return result.success && result.text.trim().length > 50 // At least 50 chars
  } catch {
    return false
  }
}
